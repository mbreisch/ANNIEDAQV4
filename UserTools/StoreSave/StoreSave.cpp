...
bool StoreSave::Initialise(std::string configfile, DataModel &data){
  ...
  if(m_verbose>1){
    for(std::map<std::string, unsigned long>::iterator it=m_trigger_values.begin(); it!=m_trigger_values.end(); it++){
  ...
  args->monitoring= new zmq::socket_t(*m_data->context, ZMQ_PUB);
  args->monitoring->bind("tcp://*:33333");           
  
  m_util->AddService("MonitorData", 33333);
...
bool StoreSave::Execute(){
  ...
  for(std::map<std::string,unsigned long>::iterator it=m_data->triggers.begin(); it!= m_data->triggers.end(); it++){
    ...
    status<<it->first<<"="<<it->second<<":"<<m_data->buffers[it->first]<<", ";

    if(m_trigger_values.count(it->first)){
      if(m_verbose>1) std::cout<<"Trigger "<<it->first<<" "<<it->second<<"%"<<m_trigger_values[it->first]<<" >= "<<m_previous_values[it->first]<<std::endl;
	...
	if(m_verbose>1)std::cout<<"triggered readout "<<it->first<<" , "<<it->second<<std::endl;
	m_previous_values[it->first]=0;
	if(m_verbose>1)std::cout<<"trigger before "<< args->trigger<<std::endl;
	args->mtx.lock();
	args->trigger=true;
	args->mtx.unlock();
	if(m_verbose>1)std::cout<<"trigger after "<< args->trigger<<std::endl;
      }
  ...
  status<<"R:S:P="<<m_data->run<<":"<<m_data->subrun<<":"<<args->part;
...
void StoreSave::Thread(Thread_args* arg){
  ...
  if(args->trigger){
    ...
    tmppath<<"/data/"<<args->OutFile<<"R"<<args->run<<"S"<<args->subrun<<"p"<<args->part;
    ...
    zmq::message_t key(9);
    snprintf ((char *) key.data(), 9 , "%s" , "DataFile");
    args->monitoring->send( key, ZMQ_SNDMORE);
    zmq::message_t path(tmppath.str().length()+1);
    snprintf ((char *) path.data(), tmppath.str().length()+1 , "%s" , tmppath.str().c_str());
    args->monitoring->send(path);
